...
// --- FUNGSI HELPER BARU UNTUK MENDAPATKAN NOMOR MINGGU ---
...
// BAGIAN 3: LOGIKA LOGIN & LOGOUT (Tidak Berubah)
...
function openAdminTab(evt, tabName) {
    ...
    else if (tabName === 'JadwalPelajaran') renderAdminManajemenJadwal(); // <-- PANGGIL FUNGSI BARU
    else if (tabName === 'Pengumuman') renderAdminPengumuman();
}

// --- FITUR BARU: MANAJEMEN JADWAL PELAJARAN OLEH ADMIN ---
...
function renderTabelJadwalAdmin(id_kelas) {
    ...
    let html = `<table class="jadwal-table">
        <thead><tr><th>Hari</th><th>Jam Mulai</th><th>Jam Selesai</th><th>Mata Pelajaran</th><th>Aksi</th></tr></thead>
        <tbody>`;
    
    jadwalKelas.forEach((sesi, index) => {
        html += `<tr>
            <td>${['Minggu','Senin','Selasa','Rabu','Kamis','Jumat'][sesi.hari]}</td>
            <td>${sesi.jamMulai}</td><td>${sesi.jamSelesai}</td><td>${sesi.mataPelajaran}</td>
    ...
    html += `<div class="form-container">
        ...
        <div class="jadwal-form">
            ...
            <select id="jadwal-hari-baru">${hariOptions.map((h, i) => `<option value="${i+1}">${h}</option>`).join('')}</select>
...
// --- FITUR BARU: CATATAN PR UNTUK SISWA ---
...
function cekDanHapusCatatanLama() {
    ...
    data.catatanPR = data.catatanPR.filter(catatan => {
        // Hapus hanya jika catatan milik siswa ini DAN dibuat di minggu sebelumnya
        return !(catatan.id_siswa === currentUser.id && catatan.mingguDibuat < mingguSekarang);
    });
}
...
function renderJadwalSiswa() {
    ...
    if (jadwalKelas.length === 0) {
        container.innerHTML = '<p>Jadwal pelajaran untuk kelas ini belum diatur oleh admin.</p>';
        return;
    }
        ...
        if (sesiUntukHariIni.length > 0) {
            sesiUntukHariIni.forEach(sesi => {
                ...
                html += `<div class="jadwal-sesi">
                    ...
                    <textarea class="catatan-pr" 
                              id="catatan-${sesi.id}" 
                              placeholder="Ketik catatan PR di sini..."
