...
// BAGIAN 4: RENDER DASHBOARD UTAMA (DIPERBARUI)
...
function tambahPesanDiskusi(id_tugas){const pesan=document.getElementById(`pesan-diskusi-${id_tugas}`).value;if(!pesan.trim())return;const tugas=data.tugas.find(t=>t.id===id_tugas);tugas.diskusi.push({id_user:currentUser.id,nama_user:currentUser.nama,role:currentRole,pesan,timestamp:new Date}),"siswa"===currentRole?createNotification(tugas.id_guru,"guru",`Siswa '${currentUser.nama}' mengirim pesan di tugas '${tugas.judul}'.`):"guru"===currentRole&&data.users.siswas.filter(s=>s.id_kelas===tugas.id_kelas).forEach(s=>{createNotification(s.id,"siswa",`Guru '${currentUser.nama}' membalas di diskusi tugas '${tugas.judul}'.`)}),"siswa"===currentRole?renderDaftarTugas():"guru"===
