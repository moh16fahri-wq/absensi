...
const data = {
    users: {
        ...
        gurus: [
            { id: 1, nama: "Budi Santoso", password: "guru1" }, { id: 2, nama: "Anisa Putri", password: "guru2" }
        ],
        siswas: [
    ...
    jadwalPelajaran: [
        { id: 1, id_kelas: 1, hari: 1, jam: 8, mapel: "Matematika", id_guru: 1 }, { id: 2, id_kelas: 1, hari: 2, jam: 10, mapel: "Fisika", id_guru: 2 },
        { id: 3, id_kelas: 2, hari: 1, jam: 9, mapel: "Biologi", id_guru: 2 }
    ],
    catatanPR: [],
    diskusi: [
...
function renderAdminManajemen() { document.getElementById("Manajemen").innerHTML = ` <div class="dashboard-section"> <div class="manajemen-header"> <h5>Manajemen Data Siswa</h5> <button class="small-btn" onclick="tampilkanFormManajemen('siswa')">+ Tambah Siswa</button> </div> <div id="form-manajemen-siswa" class="hidden"></div> <div class="table-container" id="tabel-siswa"></div> </div> <div class="dashboard-section"> <div class="manajemen-header"> <h5>Manajemen Data Guru</h5> <button class="small-btn" onclick="tampilkanFormManajemen('guru')">+ Tambah Guru</button> </div> <div id="form-manajemen-guru" class="hidden"></div> <div class="table-container" id="tabel-guru"></div> </div> <div class="dashboard-section"> <div class="manajemen-header"> <h5>Manajemen Data Kelas</h5> <button class="small-btn" onclick="tampilkanFormManajemen('kelas')">+ Tambah Kelas</button> </div> <div id="form-manajemen-kelas" class="hidden"></div> <div class="table-container" id="tabel-kelas"></div> </div> `; renderTabelManajemen('siswa'); renderTabelManajemen('guru'); renderTabelManajemen('kelas'); }
function tampilkanFormManajemen(tipe, id = null) { const container = document.getElementById(`form-manajemen-${tipe}`); container.classList.toggle('hidden'); if (container.classList.contains('hidden')) { container.innerHTML = ''; return; } let html = '', dataEdit = null; const isEdit = id !== null; if (tipe === 'siswa') { if(isEdit) dataEdit = data.users.siswas.find(s => s.id === id); const kelasOptions = data.kelas.map(k => `<option value="${k.id}" ${isEdit && dataEdit.id_kelas === k.id ? 'selected' : ''}>${k.nama}</option>`).join(''); html = ` <div class="manajemen-form"> <input type="text" id="manajemen-nama-siswa" placeholder="Nama Siswa" value="${isEdit ? dataEdit.nama : ''}"> <input type="text" id="manajemen-pass-siswa" placeholder="Password" value="${isEdit ? dataEdit.password : ''}"> <select id="manajemen-kelas-siswa">${kelasOptions}</select> <button onclick="simpanDataManajemen('siswa', ${id})">${isEdit ? 'Update' : 'Simpan'}</button> </div>`; } else if (tipe === 'guru') { if(isEdit) dataEdit = data.users.gurus.find(g => g.id === id); html = ` <div class="manajemen-form"> <input type="text" id="manajemen-nama-guru" placeholder="Nama Guru" value="${isEdit ? dataEdit.nama : ''}"> <input type="text" id="manajemen-pass-guru" placeholder="Password" value="${isEdit ? dataEdit.password : ''}"> <button onclick="simpanDataManajemen('guru', ${id})">${isEdit ? 'Update' : 'Simpan'}</button> </div>`; } else if (tipe === 'kelas') { if(isEdit) dataEdit = data.kelas.find(k => k.id === id); html = ` <div class="manajemen-form"> <input type="text" id="manajemen-nama-kelas" placeholder="Nama Kelas" value="${isEdit ? dataEdit.nama : ''}"> <button onclick="simpanDataManajemen('kelas', ${id})">${isEdit ? 'Update' : 'Simpan'}</button> </div>`; } container.innerHTML = html; }
function simpanDataManajemen(tipe, id = null) { const isEdit = id !== null; if (tipe === 'siswa') { const nama = document.getElementById('manajemen-nama-siswa').value; const password = document.getElementById('manajemen-pass-siswa').value; const id_kelas = parseInt(document.getElementById('manajemen-kelas-siswa').value); if(!nama || !password) return alert("Nama dan password tidak boleh kosong!"); if (isEdit) { const siswa = data.users.siswas.find(s => s.id === id); siswa.nama = nama; siswa.password = password; siswa.id_kelas = id_kelas; } else { data.users.siswas.push({ id: nextSiswaId++, nama, password, id_kelas }); } } else if (tipe === 'guru') { const nama = document.getElementById('manajemen-nama-guru').value; const password = document.getElementById('manajemen-pass-guru').value; if(!nama || !password) return alert("Nama dan password tidak boleh kosong!"); if(isEdit) { const guru = data.users.gurus.find(g => g.id === id); guru.nama = nama; guru.password = password; } else { data.users.gurus.push({ id: nextGuruId++, nama, password }); } } else if (tipe === 'kelas') { const nama = document.getElementById('manajemen-nama-kelas').value; if(!nama) return alert("Nama kelas tidak boleh kosong!"); if(isEdit) { const kelas = data.kelas.find(k => k.id === id); kelas.nama = nama; } else { data.kelas.push({ id: nextKelasId++, nama, lokasi: { latitude: 0, longitude: 0 }, backgroundData: null }); } } alert(`Data ${tipe} berhasil ${isEdit ? 'diperbarui' : 'disimpan'}!`); renderAdminManajemen(); }
function renderTabelManajemen(tipe) { const container = document.getElementById(`tabel-${tipe}`); let html = '<table><thead><tr>'; if(tipe === 'siswa') html += '<th>ID</th><th>Nama</th><th>Kelas</th><th>Aksi</th>'; else if(tipe === 'guru') html += '<th>ID</th><th>Nama</th><th>Aksi</th>'; else if(tipe === 'kelas') html += '<th>ID</th><th>Nama</th><th>Aksi</th>'; html += '</tr></thead><tbody>'; if (tipe === 'siswa') { data.users.siswas.forEach(s => { const kelas = data.kelas.find(k => k.id === s.id_kelas); html += `<tr><td>${s.id}</td><td>${s.nama}</td><td>${kelas ? kelas.nama : 'N/A'}</td><td> <button class="small-btn edit" onclick="tampilkanFormManajemen('siswa', ${s.id})">Edit</button> <button class="small-btn delete" onclick="hapusDataManajemen('siswa', ${s.id})">Hapus</button> </td></tr>`; }); } else if (tipe === 'guru') { data.users.gurus.forEach(g => { html += `<tr><td>${g.id}</td><td>${g.nama}</td><td> <button class="small-btn edit" onclick="tampilkanFormManajemen('guru', ${g.id})">Edit</button> <button class="small-btn delete" onclick="hapusDataManajemen('guru', ${g.id})">Hapus</button> </td></tr>`; }); } else if (tipe === 'kelas') { data.kelas.forEach(k => { html += `<tr><td>${k.id}</td><td>${k.nama}</td><td> <button class="small-btn edit" onclick="tampilkanFormManajemen('kelas', ${k.id})">Edit</button> <button class="small-btn delete" onclick="hapusDataManajemen('kelas', ${k.id})">Hapus</button> </td></tr>`; }); } html += '</tbody></table>'; container.innerHTML = html; }
function hapusDataManajemen(tipe, id) { if(!confirm(`Apakah Anda yakin ingin menghapus data ${tipe} ini?`)) return; if(tipe === 'siswa') data.users.siswas = data.users.siswas.filter(s => s.id !== id); else if(tipe === 'guru') data.users.gurus = data.users.gurus.filter(g => g.id !== id); else if(tipe === 'kelas') data.kelas = data.kelas.filter(k => k.id !== id); alert(`Data ${tipe} berhasil dihapus.`); renderAdminManajemen(); }
function renderAdminManajemenJadwal() { const container = document.getElementById("JadwalPelajaran"); const kelasOptions = data.kelas.map(k => `<option value="${k.id}">${k.nama}</option>`).join(''); const guruOptions = data.users.gurus.map(g => `<option value="${g.id}">${g.nama}</option>`).join(''); const hariOptions = ["Senin", "Selasa", "Rabu", "Kamis", "Jumat"].map((h, i) => `<option value="${i+1}">${h}</option>`).join(''); container.innerHTML = ` <div class="dashboard-section"> <h5>Buat Jadwal Pelajaran Baru</h5> <div class="manajemen-form"> <select id="jadwal-kelas">${kelasOptions}</select> <select id="jadwal-hari">${hariOptions}</select> <input type="number" id="jadwal-jam" placeholder="Jam ke-" min="1" max="10"> <input type="text" id="jadwal-mapel" placeholder="Mata Pelajaran"> <select id="jadwal-guru">${guruOptions}</select> <button onclick="simpanJadwalPelajaran()">Simpan Jadwal</button> </div> </div> <div class="dashboard-section"> <h5>Lihat Jadwal Pelajaran per Kelas</h5> <div class="jadwal-pelajaran-controls"> <select id="lihat-jadwal-kelas" onchange="renderTabelJadwalPelajaran(this.value)"> <option value="">Pilih Kelas</option> ${kelasOptions} </select> </div> <div class="jadwal-pelajaran-grid" id="jadwal-pelajaran-container"></div> </div> `; }
function simpanJadwalPelajaran() { const id_kelas = parseInt(document.getElementById('jadwal-kelas').value); const hari = parseInt(document.getElementById('jadwal-hari').value); const jam = parseInt(document.getElementById('jadwal-jam').value); const mapel = document.getElementById('jadwal-mapel').value; const id_guru = parseInt(document.getElementById('jadwal-guru').value); if(!id_kelas || !hari || !jam || !mapel || !id_guru) return alert("Semua field harus diisi!"); const jadwalBentrok = data.jadwalPelajaran.find(j => j.id_kelas === id_kelas && j.hari === hari && j.jam === jam); if(jadwalBentrok) return alert("Sudah ada jadwal di kelas, hari, dan jam yang sama!"); data.jadwalPelajaran.push({ id: nextJadwalPelajaranId++, id_kelas, hari, jam, mapel, id_guru }); alert("Jadwal pelajaran berhasil disimpan!"); const kelasTerpilih = document.getElementById('lihat-jadwal-kelas').value; if (kelasTerpilih == id_kelas) { renderTabelJadwalPelajaran(id_kelas); } }
function renderTabelJadwalPelajaran(id_kelas) { const container = document.getElementById('jadwal-pelajaran-container'); if (!id_kelas) { container.innerHTML = '<p>Silakan pilih kelas untuk melihat jadwal.</p>'; return; } let html = ''; const hariSekolah = [1, 2, 3, 4, 5]; const namaHari = ["", "Senin", "Selasa", "Rabu", "Kamis", "Jumat"]; hariSekolah.forEach(hari => { const jadwalHari = data.jadwalPelajaran.filter(j => j.id_kelas == id_kelas && j.hari === hari).sort((a, b) => a.jam - b.jam); html += `<div class="jadwal-hari"><h6>${namaHari[hari]}</h6>`; if (jadwalHari.length > 0) { jadwalHari.forEach(sesi => { const guru = data.users.gurus.find(g => g.id === sesi.id_guru); html += ` <div class="jadwal-sesi"> <div class="sesi-info"> <strong>${sesi.mapel}</strong> <button class="small-btn delete" style="padding: 4px 8px;" onclick="hapusJadwalPelajaran(${sesi.id})">x</button> </div> <small>Jam ke-${sesi.jam} (${guru ? guru.nama : 'N/A'})</small> </div>`; }); } else { html += '<p class="sesi-kosong">Tidak ada jadwal</p>'; } html += '</div>'; }); container.innerHTML = html; }
function hapusJadwalPelajaran(id) { if (!confirm("Yakin ingin menghapus jadwal ini?")) return; const id_kelas_dihapus = data.jadwalPelajaran.find(j => j.id === id)?.id_kelas; data.jadwalPelajaran = data.jadwalPelajaran.filter(j => j.id !== id); alert("Jadwal berhasil dihapus."); renderTabelJadwalPelajaran(id_kelas_dihapus); }
