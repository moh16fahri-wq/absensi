...
function renderAdminDashboard() {
    ...
    <div class="tabs">
        ...
        <button class="tab-link" onclick="openAdminTab(event, 'Jadwal')">🗓️ Jadwal Guru</button>
    ...
    <div id="Jadwal" class="tab-content"></div>
...
function openAdminTab(evt, tabName) {
    ...
    else if (tabName === 'Jadwal') renderAdminJadwal(); // <-- PANGGIL FUNGSI BARU
    else if (tabName === 'Pengumuman') renderAdminPengumuman();
}

// --- FITUR BARU: MANAJEMEN JADWAL OLEH ADMIN ---
function renderAdminJadwal() {
    ...
    const jamOptions = Array.from({ length: 10 }, (_, i) => i + 8); // Jam 8 pagi sampai 5 sore
    const kelasOptions = data.kelas.map(k => `<option value="${k.id}">${k.nama}</option>`).join('');

    data.users.gurus.forEach(guru => {
        let jadwalHTML = `<div class="jadwal-guru-container task-card"><h5>${guru.nama}</h5>`;
        ...
        if (guru.jadwal && guru.jadwal.length > 0) {
            ...
            jadwalHTML += `<p>Belum ada jadwal yang diatur untuk guru ini.</p>`;
        }
...
function tambahJadwalGuru(guruId) {
    ...
    const jadwalExists = guru.jadwal.some(j => j.id_kelas === id_kelas && j.hari === hari && j.jam === jam);
    if (jadwalExists) {
        alert('Jadwal ini sudah ada untuk guru tersebut.');
        return;
    }
    ...
    // Sortir jadwal berdasarkan hari dan jam
    guru.jadwal.sort((a, b) => a.hari - b.hari || a.jam - b.jam);

    renderAdminJadwal(); // Refresh tampilan
}
